...
def test_89_1_active_player_must_activate_system_without_their_command_token() -> None:
    ...
    fresh_system = System(command_tokens=())
    previously_activated_system = System(command_tokens=(CommandToken(player_name=player_a.name),))
    session = make_basic_session_from_players(players=(player_a, player_b))
    assert session.engine.apply_command(
        state=session.current_state,
        command=ActivateCommand(
            actor=player_a, command_type=CommandType.INITIATE_TACTICAL_ACTION, system=fresh_system
        ),
    ).success
    assert not session.engine.apply_command(
        ...
        command=ActivateCommand(
            ...
            system=previously_activated_system,
        ),
    ).success
